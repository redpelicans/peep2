...
const doFilter = (nfilter, notes) => filter(note => nmatch(regexp(nfilter), note.content), notes);

export const getFilter = state => state.notes.filter || '';
const getNotes = state => state.notes.data;

export const getVisibleNotes = createSelector([getFilter, getNotes], (filter, notes) => doFilter(filter, notes));
