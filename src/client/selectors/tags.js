...
const mergeTags = compose(filter(identity), reduce((tags, list) => concat(tags, getFlattenTags(list)), []));
const doSort = sort(descend(prop(1)));
const doFormat = compose(map(([tag, elts]) => [tag, elts.length]), toPairs, groupBy(identity));
const regexp = filter => new RegExp(filter, 'i');
const doFilter = tfilter => filter(x => !isEmpty(match(regexp(tfilter), x)));
const groupByFilterAndSort = (filter, ...lists) => compose(doSort, doFormat, doFilter(filter))(mergeTags(lists));
