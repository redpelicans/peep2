...
const StyledCalendar = styled.div`
  ...
  display: grid;
  grid-template-columns: ${({ days }) => ['100px', ...times(() => '35px', days.length)].join(' ')};
  grid-auto-rows: 35px;
  grid-gap: 1px;
  text-align: center;
`;
...
const StyledTooltip = styled.em`
  font-size: 0.8em;
`;

const DayHeader = ({ date, calendar }) => {
...
const MonthHeader = ({ date, calendar }) => {
...
const StyledSpareDay = styled.div`
  background-color: ${Colors.GRAY1};
`;

const SpareDay = ({ onMouseUp }) => {
  ...
  return (
    <StyledSpareDay onMouseUp={handleMouseUp} />
  );
};
...
const StyledWorkingDay = styled.div`
  background-color: ${({ selected }) => selected ? Colors.GREEN5 : Colors.DARK_GRAY4};
`;

const WorkingDay = ({ date, worker, selected, onMouseEnter, onMouseUp, onMouseDown }) => {
...
const Day = ({ calendar, date, worker, from, to, currentWorker, onMouseUp, ...props }) => {
  const selected = betweenDates(date, from, to) && currentWorker === worker;
  if (isWorkingDay(calendar, date)) return <WorkingDay selected={selected} date={date} worker={worker} onMouseUp={onMouseUp} {...props} />;
...
class Calendar extends Component {
  ...
  render() {
    ...
    const daysheader = map(day => <DayHeader key={day} date={day} calendar={calendar} />, days);
