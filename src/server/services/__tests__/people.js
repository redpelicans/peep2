...
describe("People service", () => {
  ...
  it("expect add", done => {
    ...
    const checkPerson = person => {
      ...
      expect(
        R.omit(["_id", "companyId", "createdAt", "constructor"], person)
      ).toEqual(res);
      return person;
    };
  ...
  it("expect toggle preferred", done => {
    ...
    service
      .add(newPerson, { user })
      .then(person =>
        service.setPreferred({ _id: person._id, preferred: true }, { user })
      )
      .then(checkIsPreferred)
      .then(person =>
        service.setPreferred({ _id: person._id, preferred: false }, { user })
      )
  ...
  it("expect update", done => {
    ...
    const checkObj = obj => {
      ...
      expect(
        R.omit(
          ["_id", "companyId", "updatedAt", "createdAt", "constructor"],
          obj
        )
      ).toEqual(res);
      return obj;
  ...
  it("expect update tags", done => {
    ...
    service
      .add(newObj, { user })
      .then(o =>
        service.update({ ...o, tags: ["tag11", "tag3", "tag2"] }, { user })
      )
      .then(checkObj)
      .then(() => done())
      .catch(done);
